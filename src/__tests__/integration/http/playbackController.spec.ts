...
beforeEach(() => {
	...
	new PlaybackController(server, cachePortMock, authMiddlewareMock, loggerPortMock)
...
describe('PlaybackController', () => {
	describe('PUT /playback/state', () => {
		it('should return 200 and set playback state', async () => {
			...
			expect(cachePortMock.setPlaybackState).toHaveBeenCalledWith(
				'user-id',
	...
	describe('GET /playback/state', () => {
		...
		it('should return null shape when no state exists', async () => {
			...
			expect(res.status).toBe(200)
			expect(res.body.data).toEqual({ trackId: null, positionMs: 0, updatedAt: null })
		})
