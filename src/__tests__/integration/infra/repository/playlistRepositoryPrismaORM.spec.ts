import { createDummyPlaylistPrismaORM } from '#__tests__/factories/PlaylistFactory.js'
...
describe('PlaylistRepositoryPrisma', () => {
	...
	describe('listByOwnerId', () => {
		it('should return playlists owned by the given user', async () => {
			const user2 = await createDummyUserPrismaORM(prisma, {
				name: 'User 2',
	...
	describe('addTrack', () => {
		it('should connect a track to the playlist', async () => {
			...
			await playlistRepository.addTrack(dummyPlaylist.id, track.id)
	...
	describe('removeTrack', () => {
		it('should disconnect a track from the playlist', async () => {
			...
			await playlistRepository.removeTrack(dummyPlaylist.id, track.id)
		...
		it('should not throw when disconnecting a non-existent track', async () => {
			...
			await expect(
				playlistRepository.removeTrack(dummyPlaylist.id, crypto.randomUUID()),
